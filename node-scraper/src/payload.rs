...
impl Payload {
    fn cluster_aggregator_endpoint(&self) -> String {
        match self {
    ...
    pub async fn push_to_cluster_aggregator(
        &self,
        namespace: &str,
        service: &str,
        ...
        let url = format!(
            ...
            self.cluster_aggregator_endpoint()
        );
        debug!(url = %url, "relaying payload to cluster-aggregator");
        trace!(
            payload = ?self,
            "payload being sent"
        );
        let response = client
            ...
            .header(
                "X-Agent-Git",
                option_env!("CHECKMK_KUBERNETES_AGENT_GIT_SHA").unwrap_or("unknown"),
            )
            .send()
            .await?;
        if response.status().is_success() {
            debug!(status = %response.status(), "payload accepted by cluster-aggregator");
        } else {
            warn!(status = ?response, "payload rejected by cluster-aggregator");
        }
        Ok(response)
    }
}
