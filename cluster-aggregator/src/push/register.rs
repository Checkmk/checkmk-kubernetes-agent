//! This module handles registration between cluster-aggregator and the Checkmk push
//! agent receiver.
//!
    ...
    pub async fn renew(&self, client: &CheckmkPushClient) -> Result<CheckmkPushClient> {
        ...
        if &secret_uuid != client.uuid() {
            return Err(push::Error::PushMode(
                "push certificate secret changed while cluster-aggregator was running".to_string(),
            )
            .into());
        }
