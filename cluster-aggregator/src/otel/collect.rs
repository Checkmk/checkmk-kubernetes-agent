...
pub(super) fn collect_entities(
    stat_summaries: impl IntoIterator<Item = Arc<NodeScraperIngestion<StatsSummary>>>,
    cluster_name: &str,
    owner_graph: &OwnerGraph,
) -> Vec<KubeEntity> {
...
mod tests {
    ...
    fn ingestion(pods: Vec<Pod>) -> Arc<NodeScraperIngestion<StatsSummary>> {
        Arc::new(NodeScraperIngestion {
