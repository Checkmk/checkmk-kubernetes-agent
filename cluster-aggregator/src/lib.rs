...
//! <div class="warning">
//!
//! This documentation is meant for **developers** of Checkmk Kubernetes Agent
//! and for community members wishing to contribute to the project. It is
//! expressly **not** meant as an end-user guide for setting up Kubernetes
//! monitoring in Checkmk.
//!
...
//!
//! You are looking at the documentation for one component of the agent (out of
//! two): `cluster-aggregator`.
//!
//! `cluster-aggregator` is the heart of Checkmk's Kubernetes monitoring. It
//! collects information from various sources, namely the Kubernetes API and the
//! `node-scrapers` (the "other" component). It takes the collected information
//! and makes it available to Checkmk in the form of pre-generated, pre-computed
//! Checkmk sections (those `<<<foo>>>` things).
//!
//! ## How sections get generated
//!
//! 1. Information enters `cluster-aggregator` from several sources:
...
//!    - **The `node-scraper`s**: real-time usage data (how much RAM/CPU is
...
//!      openwrt agent... or at least some of it). Each `node-scraper`
