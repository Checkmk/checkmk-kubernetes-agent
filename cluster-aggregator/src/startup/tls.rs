//! Bootstrap logic for cluster-aggregator TLS listeners.
//!
...
//!
//! Generating certificates in cluster-aggregator rather than during Helm rendering
...
//! ingestion listener used by node-scraper.
...
fn generate_tls_material(dns_names: Vec<String>, longevity: Duration) -> Result<TlsMaterial> {
    ...
    ca_params
        .distinguished_name
        .push(DnType::CommonName, "Checkmk Kubernetes Agent internal CA");
    ...
    server_params
        .distinguished_name
        .push(DnType::CommonName, "cluster-aggregator");
...
mod tests {
    ...
    fn generated_tls_material_has_expected_identity_and_validity() {
        let dns_name = "cluster-aggregator.monitoring.svc";
