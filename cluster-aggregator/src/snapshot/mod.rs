...
use crate::snapshot::self_health::{NodeScraperDaemonSet, SelfHealth};
...
impl Snapshot {
    ...
    pub fn new(
        ...
        node_scraper_daemonset: Option<&NodeScraperDaemonSet>,
    ) -> Self {
        ...
        let expected_node_scraper_nodes = SelfHealth::expected_node_names(
            &stores.daemonsets,
            &owner_graph,
            node_scraper_daemonset,
        );
        let self_health = SelfHealth::new(
            instant,
            &expected_node_scraper_nodes,
            reflector_healths,
