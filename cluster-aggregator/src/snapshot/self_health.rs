...
/// A point-in-time view of the health of Checkmk Kubernetes Agent and its
/// components: cluster-aggregator and node-scraper.
///
/// This is included in the [`crate::snapshot::Snapshot`] and used to generate
/// cluster-level sections for alerting about the state of the agent, such as
/// when a node last reported in (node-scraper), or if the reflectors had
/// errors.
///
/// Scheduled Pods belonging to the node-scraper DaemonSet are the source of
...
/// cache entries for broken node-scraper Pods.
#[derive(Debug, Default)]
pub struct SelfHealth {
    /// Health data for each of the payloads sent by node-scraper, keyed by
    /// node name.
    pub node_node_scrapers: BTreeMap<String, NodeNodeScraperHealth>,
    /// Maps kind names to reflector states.
    pub(crate) reflector_healths: BTreeMap<&'static str, ReflectorHealth>,
    /// Build information about cluster-aggregator.
    pub cluster_aggregator_build_info: ClusterAggregatorBuildInfo,
}

/// Self-health data for the node-scraper running on a single node.
#[derive(Debug, Default)]
pub struct NodeNodeScraperHealth {
    pub kubelet_stats: NodeScraperIngestionHealth,
    pub kubelet_health: NodeScraperIngestionHealth,
    pub system_agent: NodeScraperIngestionHealth,
}

/// Freshness and metadata for one kind of node-scraper ingestion.
#[derive(Debug, Default)]
pub struct NodeScraperIngestionHealth {
...
/// Identifies the node-scraper DaemonSet in the reflector store.
#[derive(Clone, Debug)]
pub struct NodeScraperDaemonSet {
...
#[derive(Debug, Default)]
pub struct ClusterAggregatorBuildInfo {
...
impl SelfHealth {
    pub(crate) fn new(
        ...
        kubelet_stats_summary_cache: &Cache<String, Arc<NodeScraperIngestion<StatsSummary>>>,
        kubelet_health_cache: &Cache<String, Arc<NodeScraperIngestion<KubeletHealth>>>,
        system_agent_cache: &Cache<String, Arc<NodeScraperIngestion<SystemAgentOutput>>>,
    ) -> SelfHealth {
        ...
        let node_node_scrapers = kubelet_stats
            .into_iter()
        ...
        let cluster_aggregator_build_info = ClusterAggregatorBuildInfo {
            ...
            git_sha: option_env!("CHECKMK_KUBERNETES_AGENT_GIT_SHA"),
        };
        SelfHealth {
            node_node_scrapers,
            reflector_healths,
    ...
    pub(crate) fn expected_node_names(
        ...
        identity: Option<&NodeScraperDaemonSet>,
    ) -> BTreeSet<String> {
    ...
    /// node-scraper ingestions, calculate how long it has been since each
    ...
    fn cache_health<T>(
        ...
        cache: impl IntoIterator<Item = (Arc<String>, Arc<NodeScraperIngestion<T>>)>,
    ) -> BTreeMap<String, NodeScraperIngestionHealth> {
        let mut intermediary: HashMap<String, NodeScraperIngestionHealth> = cache
            .into_iter()
            .map(|(name, ingestion)| {
                (
                    name.to_string(),
                    NodeScraperIngestionHealth {
...
mod tests {
    ...
    use crate::ingest::NodeScraperMetadata;
    use crate::test_support::*;

    fn ingestion(instant: Instant) -> Arc<NodeScraperIngestion<()>> {
        NodeScraperIngestion {
            received_at: instant,
            metadata: NodeScraperMetadata::default(),
            payload: (),
        }
        .into()
    }
    ...
    #[test]
    fn expected_nodes_are_scheduled_node_scraper_pods() {
        let mut daemonset = daemonset("node-scraper");
        ...
        let identity = NodeScraperDaemonSet {
